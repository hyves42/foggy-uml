...
pub fn starts_with_token<'a, 'b>(input: &'a str, tokens: &'b [&str]) -> bool {
...
pub fn consume_until_token_escape<'a, 'b>(
    ...
    let mut all_tokens: Vec<&str> = escaped_tokens.iter().map(|s| s.as_str()).collect();
...
mod tests {
    ...
    fn test_consume_with_escape() {
        assert_eq!(
            consume_until_token_escape("toto\\\"\"", &["\""]),
            Ok(("\"", "toto\\\""))
        );
        assert_eq!(
            consume_until_token_escape("toto\\*tata*titi", &["\"", "*"]),
            Ok(("*titi", "toto\\*tata"))
        );
        ...
        assert_eq!(
            consume_until_token_escape("toto*tata", &["\"", "*"]),
            Ok(("*tata", "toto"))
        );
        // test utf-8
        assert_eq!(
            consume_until_token_escape("きゃりー\\*ぱみゅ*ぱみゅ", &["\"", "*"]),
            Ok(("*ぱみゅ", "きゃりー\\*ぱみゅ"))
        );
    }
    ...
    fn test_consume_nominal_between_tokens() {
        ...
        //utf-8
        assert_eq!(
            consume_between_tokens("**きゃりー**ぱみゅぱみゅ", &["**"]),
            Ok(("ぱみゅぱみゅ", "きゃりー", "**きゃりー**".len()))
        );
    }
