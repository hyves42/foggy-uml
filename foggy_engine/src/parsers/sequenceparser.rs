...
impl SequenceDiagramParser {
    ...
    fn add_participant<'a>(&mut self, input: &str, participant_token: &str) -> Result<(), String> {
        let mut slice = input;
    ...
    fn add_box<'a>(&mut self, input: &str) -> Result<(), String> {
        let mut slice = input;
        ...
        self.open_header_tokens
            .push((Rc::clone(&ptr), HDCloseCondition::EndBox));
    ...
    > {
        ...
        let map: std::collections::HashMap<&str, (ArrowDirection, ArrowLineType, ArrowType)> = hashmap![
                ...
                Some((direction, line_type, a_type)) => {
                    ...
                    if arrow_direction == ArrowDirection::Bidirectional && arrow_decor != None {
                        return Err(String::from(
                            "Invalid arrow : bidirectional arrows can't have any decoration",
                        ));
                    }
                    // Previously set decoration is only valid if the arrow goes left
                    if arrow_direction == ArrowDirection::Right && arrow_decor != None {
                        return Err(String::from(
                            "Invalid arrow : decoration before right arrow must be on right ride",
                        ));
                    }
                    slice = remaining;
                }
...
impl Parser for SequenceDiagramParser {
        ...
        if self.state == SequenceDiagramParserState::Header {
            ...
            if let Ok((new_slice, token)) = consume_token_in_list(slice, &RESERVED_TOKENS_HEADER) {
                ...
                match token {
                    ...
                    "box" => {
                        self.add_box(input.slice);
                    }
                    "end box" => {
                        self.end_box();
                    }
                    "title" => {
                        return Err((input, String::from("runtime error, invalid condition")))
                    }
                    "hide" => {
                        return Err((input, String::from("runtime error, invalid condition")))
                    }
    ...
    fn flush(&mut self) -> (Vec<Rc<RefCell<Element>>>, Vec<Rc<RefCell<Document>>>) {
        ...
        return (
            vec![
                Rc::new(RefCell::new(header_element)),
                Rc::new(RefCell::new(content_element)),
            ],
            vec![],
        );
    }
...
mod tests {
    ...
    fn test_sequenceparser_header1() {
        ...
        let expected: Vec<Rcc<Element>> = vec![
            rcc(Element::new("sequencediagram:header").child(
                Element::new("participant")
                    .attr("alias", "bob")
                    .child(Element::new("name").child(Element::str("bob"))),
            )),
    ...
    fn test_sequenceparser_header2() {
        ...
        let expected: Vec<Rcc<Element>> = vec![
            rcc(Element::new("sequencediagram:header").child(
                Element::new("participant")
                    .attr("alias", "bob")
                    .child(Element::new("name").child(Element::str("bobby"))),
            )),
            rcc(Element::new("sequencediagram:content")),
        ];
    ...
    fn test_sequenceparser_header3() {
        ...
        let expected: Vec<Rcc<Element>> = vec![
            rcc(Element::new("sequencediagram:header").child(
                Element::new("participant")
                    .attr("alias", "bob")
                    .child(Element::new("name").child(Element::str("bobby the 🐶"))),
            )),
            rcc(Element::new("sequencediagram:content")),
        ];
    ...
    fn test_sequenceparser_box1() {
        ...
        let expected: Vec<Rcc<Element>> = vec![
            rcc(Element::new("sequencediagram:header").child(
                Element::new("box").child(Element::new("name").child(Element::str("boxxy"))),
            )),
            rcc(Element::new("sequencediagram:content")),
        ];
    ...
    fn test_sequenceparser_box2() {
        ...
        let expected: Vec<Rcc<Element>> = vec![
            rcc(Element::new("sequencediagram:header").child(
                Element::new("box")
                    .child(Element::new("name").child(Element::str("boxxy")))
                    .child(
                        Element::new("participant")
                            .attr("alias", "bob")
                            .child(Element::new("name").child(Element::str("bob"))),
    ...
    fn test_sequenceparser_box3() {
        ...
        let expected: Vec<Rcc<Element>> = vec![
            rcc(Element::new("sequencediagram:header")
                .child(Element::new("box").child(Element::new("name").child(Element::str("boxxy"))))
                .child(
                    Element::new("participant")
                        .attr("alias", "bob")
                        .child(Element::new("name").child(Element::str("bob"))),
        ...
        let expected: Vec<Rcc<Element>> = vec![
            rcc(Element::new("sequencediagram:header")
                .child(
                    Element::new("participant")
                        .attr("alias", "alice")
                        .child(Element::new("name").child(Element::str("alice"))),
                )
                .child(
                    Element::new("participant")
                        .attr("alias", "bob")
