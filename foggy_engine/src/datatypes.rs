...
mod tests {
    ...
    fn test_recurse() {
        ...
        recurse_element_tree(Rc::clone(&tree), |e, d| {
            if e.borrow().is_text() {
                str_count += 1;
            } else {
                struct_count += 1;
    ...
    fn test_xml4() {
        ...
        assert_eq!(
            element.to_xml(),
            String::from("<img src=\"surprised_pikachu.jpg\"/>\n")
        );
    }
    ...
    fn test_xpath1() {
        let body = Element::new("body").child(
            Element::new("p").child(Element::new("span").child(Element::str("lorem ipsum"))),
        );

        assert_eq!(
            body.get("p/span/text()"),
            Some(QueryResult::Text(String::from("lorem ipsum")))
        );
    }
}
