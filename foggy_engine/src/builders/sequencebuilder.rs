...
impl SequenceDiagramBuilder {
    ...
    pub fn estimate_text_size(element: Rc<RefCell<Element>>) -> (f32, f32) {
        ...
        return (
            max_line_width as f32 * CHAR_AVERAGE_RATIO * FONT_SIZE,
            lines_counter as f32 * FONT_INTERLINE,
        );
    }
    ...
    pub fn estimate_arrow_dimensions(element: Rc<RefCell<Element>>) -> (f32, f32) {
        ...
        if let ElementContent::Tree(content) = &elt.content {
            match content.children.first() {
                // text is not mandatory on arrows
                None => return (6.0, 6.0),
                Some(e) => {
    ...
    pub fn generate_svg(&mut self, description: &[Rc<RefCell<Element>>]) -> Result<String, String> {
        ...
        let mut participants_map: HashMap<String, (Rc<RefCell<Element>>, ParticipantExtraInfo)> =
            HashMap::new();
        ...
        recurse_element_tree(Rc::clone(&header), |e, d| {
            let elt = e.borrow();
        ...
        let mut document_height: f32 = PARTICIPANT_BOX_HEIGHT; // start value
                                                               ...
                                                               // - height of document
        recurse_element_tree(Rc::clone(&content), |e, _d| {
            let elt = e.borrow();
            // if element is a participant definition
            if elt.is_tree() && elt.get_tag() == "arrow" {
        ...
        let mut x = 40.0;
        recurse_element_tree(Rc::clone(&header), |e, _d| {
            let elt = e.borrow();
            if elt.is_tree()
                && PARTICIPANTS_TYPES.contains(&elt.get_tag().as_str())
                && elt.get_attr("alias") != None
        ...
        recurse_element_tree(Rc::clone(&header), |e, _d| {
            ...
            if PARTICIPANTS_TYPES.contains(&elt.get_tag().as_str()) && elt.get_attr("alias") != None
            {
                if let Some((_, info)) = participants_map.get_mut(&elt.get_attr("alias").unwrap()) {
                    ...
                    if let Some(QueryResult::Text(text)) = elt.get("name/text()") {
                        ...
                        xml_stack
                            ...
                            .unwrap()
                            .borrow_mut()
                            .push(Rc::new(RefCell::new(text_elt)));
                    }
