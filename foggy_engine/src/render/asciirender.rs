...
fn direction(x1: usize, y1: usize, x2: usize, y2: usize) -> LineDirection {
    let w: isize = x2 as isize - x1 as isize;
    let h: isize = y2 as isize - y1 as isize;
...
impl AsciiRenderer {
        ...
        if round {
            ...
            }
            {
                ...
                self.set_char(
                    x + w - 1,
                    y + h - 1,
                    merge_char(BOX_LIGHT_ARC_UP_AND_LEFT, c),
                );
            }
        } else {
            {
    ...
    fn draw_curved_arrow(&mut self, points: &[(usize, usize)], end1: bool, end2: bool) {
        if points.len() < 2 {
            return;
        }
        let (mut x1, mut y1) = points[0];
        let mut last_dir: LineDirection = LineDirection::None;
        for (x2, y2) in points[1..].iter() {
            let new_dir = direction(x1, y1, *x2, *y2);
            self.draw_line(x1, y1, *x2, *y2);
            match last_dir {
                LineDirection::Top => match new_dir {
                    LineDirection::Left => self.set_char(x1, y1, BOX_LIGHT_ARC_DOWN_AND_LEFT),
                    LineDirection::Right => self.set_char(x1, y1, BOX_LIGHT_ARC_DOWN_AND_RIGHT),
                    _ => {}
                },
                LineDirection::Bottom => match new_dir {
                    LineDirection::Left => self.set_char(x1, y1, BOX_LIGHT_ARC_UP_AND_LEFT),
                    LineDirection::Right => self.set_char(x1, y1, BOX_LIGHT_ARC_UP_AND_RIGHT),
                    _ => {}
                },
                LineDirection::Left => match new_dir {
                    LineDirection::Top => self.set_char(x1, y1, BOX_LIGHT_ARC_UP_AND_RIGHT),
                    LineDirection::Bottom => self.set_char(x1, y1, BOX_LIGHT_ARC_DOWN_AND_RIGHT),
                    _ => {}
                },
                LineDirection::Right => match new_dir {
                    LineDirection::Top => self.set_char(x1, y1, BOX_LIGHT_ARC_UP_AND_LEFT),
                    LineDirection::Bottom => self.set_char(x1, y1, BOX_LIGHT_ARC_DOWN_AND_LEFT),
                    _ => {}
                },
                _ => {}
            }
        ...
        if (end1) {
            let (x1, y1) = points[0];
            let (x2, y2) = points[1];
            let arrow_dir = direction(x1, y1, x2, y2);
            match arrow_dir {
                LineDirection::Top => self.set_char(x1, y1, '▽'),
                LineDirection::Bottom => self.set_char(x1, y1, '△'),
                LineDirection::Left => self.set_char(x1, y1, '▷'),
                LineDirection::Right => self.set_char(x1, y1, '◁'),
                _ => {}
            }
        }
        if (end2) {
            ...
            let arrow_dir = direction(x1, y1, x2, y2);
            match arrow_dir {
                LineDirection::Top => self.set_char(x2, y2, '△'),
                LineDirection::Bottom => self.set_char(x2, y2, '▽'),
                LineDirection::Left => self.set_char(x2, y2, '◁'),
                LineDirection::Right => self.set_char(x2, y2, '▷'),
                _ => {}
            }
        }
    }
    ...
    fn draw(&mut self, object: &RenderableItem) {
        match object {
            ...
            RenderableItem::Arrow {
                ...
                end1,
                end2,
            } => self.draw_arrow(*x1, *y1, *x2, *y2, *end1, *end2),
            RenderableItem::CurvedArrow { points, end1, end2 } => {
                self.draw_curved_arrow(points, *end1, *end2)
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_draw_line_vertical2() {
        let mut renderer = AsciiRenderer::new();

        renderer.init_scene(4, 4);
        renderer.draw_line(0, 3, 0, 0);
    ...
    #[test]
    fn test_draw_line_horizontal2() {
        let mut renderer = AsciiRenderer::new();

        renderer.init_scene(4, 4);
        renderer.draw_line(3, 0, 0, 0);
    ...
    fn test_merge_char() {
        ...
        for i in 0..old.chars().count() {
            for j in 0..new.chars().count() {
                println!(
                    "old :{}, new :{}, expected:{}",
                    old.chars().nth(i).unwrap(),
                    new.chars().nth(j).unwrap(),
                    matrix[i].chars().nth(j).unwrap()
                );
                assert_eq!(
                    merge_char(new.chars().nth(j).unwrap(), old.chars().nth(i).unwrap()),
                    matrix[i].chars().nth(j).unwrap()
                );
            }
        }
    ...
    #[test]
    fn line_directions() {
        assert_eq!(direction(1, 1, 1, 1), LineDirection::None);
        assert_eq!(direction(1, 1, 3, 1), LineDirection::Right);
        assert_eq!(direction(3, 1, 1, 1), LineDirection::Left);
        assert_eq!(direction(1, 1, 1, 3), LineDirection::Bottom);
        assert_eq!(direction(1, 3, 1, 1), LineDirection::Top);

        assert_eq!(direction(1, 2, 3, 1), LineDirection::Right);
        assert_eq!(direction(3, 1, 1, 2), LineDirection::Left);
        assert_eq!(direction(2, 1, 1, 3), LineDirection::Bottom);
        assert_eq!(direction(1, 3, 2, 1), LineDirection::Top);
        assert_eq!(direction(0, 0, 4, 0), LineDirection::Right);
    }

    #[test]
    fn test_draw_curved_arrow() {
        let mut renderer = AsciiRenderer::new();

        renderer.init_scene(5, 5);
        renderer.draw_curved_arrow(
            &[(0, 0), (4, 0), (4, 3), (2, 3), (2, 2), (0, 2)],
            false,
            false,
        );
        let out = renderer.render();
    ...
    #[test]
    fn test_draw_curved_arrow3() {
        let mut renderer = AsciiRenderer::new();

        renderer.init_scene(5, 5);
        renderer.draw_curved_arrow(&[(0, 0), (0, 2), (4, 2), (4, 4)], true, true);
        let out = renderer.render();
        assert_eq!(out, "△    \n│    \n╰───╮\n    │\n    ▽\n");
    }
}
